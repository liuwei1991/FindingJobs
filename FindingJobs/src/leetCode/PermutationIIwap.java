...
public class PermutationIIwap {
	...
	public List<List<Integer>> permuteUnique(int[] num) {
		...
		Arrays.sort(num);
		this.getPermute(0, new ArrayList<Integer>(), num);
		return result;
	}
	
	private void getPermute(int level,List<Integer> sequence,int[] num){
		if(level == num.length){
		...
		for(int i=0;i<num.length;i++){
			...
			sequence.add(num[i]);
			flags[i] = true;
			getPermute(level+1,sequence,num);
			flags[i] = false;
...
//		if (level == num.length) {
//			List<Integer> tmp = new ArrayList<Integer>(sequence);
////			for (int a : sequence) {
////				tmp.add(a);
////			}
//			result.add(tmp);
